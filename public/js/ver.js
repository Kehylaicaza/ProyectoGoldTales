...
function indiceCuentosPorUsuario(id) {

    $("#porUser").empty();
    var us ={id:id };
    console.log("soy "+us.id);
         peticionCuentoporUser(us,function (result) {      
             $.each(result, function (index, elem) {
            peticionImagenes(elem, function (imagen) {
                //  alert("Pinterest amigos")
                var img = imagen[0];
                ...
                if (img != undefined) {
                     console.log("hola soy " + imagen[0].src);
                    $("#porUser").append('<li><a id=' + elem.id + '><h2 class="myTitle">' + elem.title + '</h2><p class="myId">' + elem.id + '</p><div class="col-lg-4"> <img class="imagenesCuentos"  src=' + imagen[0].src + '></div></li>');
                }
            });
                 });
         });
...
//ver imagenes por usuario
function peticionCuentoporUser(porUser, callback) {
     var datos = "";
     console.log("soy x2"+ porUser.id)
    $.ajax({
        url: '/cargarCuentoPorUser',
        type: 'POST',
        data: porUser,
        cache: false,
...
$(document).ready(function () {
 ...
 comboUsers(function (result) {
        for(var i=0;i<result.length;i++){
            $("#selectUser").append('<option value="' + result[i].id + '">' + result[i].username + '</option>');            
        }
    });
$('#selectUser').change(function() {
		alert($(this).val());
		indiceCuentosPorUsuario($(this).val())
	});
    ...
    $('body').on('click', '#principal a', function () {
      ...
      $('body').on('click', '#porUser a', function () {
        modal.style.display = "block";
        var v = $(this).attr('id');
        alert("Has seleccionado el cuento " + v);
        mostrarCuento(v);
