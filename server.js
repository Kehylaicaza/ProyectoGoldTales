...
app.get('/cuentosPorUsuario', function (req, res) {
    res.render('partials/paginas/cuentosPorUsuario');
});
...
app.post('/guardarUsuarioActualizado', (req, res) => {
    var client = new pg.Client(conString);
    client.connect(function (err) {
        if (err) {
            return console.error('could not connect to postgres', err);
...
app.post('/cargarCuentoPorUser', (req, res) => {
    var client = new pg.Client(conString);
      console.log("soy rw"+req.body.id );
    client.connect(function(err) {
        if(err) {
            return console.error('could not connect to postgres', err);
            return res.status(500).json({success: false, data: err});
        ...
        client.query('SELECT * FROM stories WHERE user_id='+ req.body.id +';', function(err, result) {
...
app.post('/cargarUsuarioPorId', (req, res) => {
    var client = new pg.Client(conString);
    client.connect(function(err) {
        if(err) {
            return console.error('could not connect to postgres', err);
        ...
        client.query('SELECT * FROM users WHERE id='+ req.body.iduser +';', function(err, result) {
            if(err) {
                return console.error('error running query', err);
            }
